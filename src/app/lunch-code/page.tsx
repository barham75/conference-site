...
export default function LunchPassPage() {
  ...
  async function showCode() {
    ...
    try {
      ...
      if (!res.ok || !data?.ok) {
        ...
        if (err.includes("please register")) {
          setMsg({ type: "err", text: "يرجى التسجيل أولاً ثم العودة لهذه الصفحة." });
        } else if (err === "not registered") {
  ...
  return (
    <div style={container}>
      ...
      <div style={tabs}>
        <button type="button" onClick={() => setDay("1")} style={day === "1" ? tabActive : tab}>
          Day 1
        </button>
        <button type="button" onClick={() => setDay("2")} style={day === "2" ? tabActive : tab}>
          Day 2
        </button>
      </div>

      <div style={card}>
        <button style={btn} disabled={loading} onClick={showCode}>
